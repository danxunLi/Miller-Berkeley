...
%   into a binary image using IMQUANTIZE.
%
...
%
%   See also GRAYTHRESH, IM2BW, IMQUANTIZE, RGB2IND.
...
if (isempty(A))
    warning(message('images:multithresh:degenerateInput',num2str(N)))
...
% Scale A to [0-1]
idxFinite = find(isfinite(A));
% If there are finite elements, then scale them between [0-1]. 
if ~isempty(idxFinite)    
    ...
    % One of many possibilities: all Infs, all -Infs, all NaNs, mixture of Infs
...
boundaries = [round(thresh)+1 num_bins]; % 'thresh' has intensities [0-255], but 'boundaries' are the indices [1 256]

% Constrain 'thresh' to be strictly increasing
...
sclThresh = cast(sclThresh,class(minA));
...
if isempty(uniqueVals)
    thresh = cast(1:N,class(uniqueVals));
    return;
end
...
if (thNeeded1 > 0)
    ...
    if (uniqueVals(1) > 1)
        ...
        thresh = [cast(1:min(thNeeded1,ceil(uniqueVals(1))-1), ...
                  class(uniqueVals)) thresh];        
    end
