...
[A, levels, values] = parse_inputs(varargin{:});
...
function [A, levels, values] = parse_inputs(varargin)

A    = varargin{1};
validateattributes(A,{'numeric'},{'nonsparse','real', 'nonnan'}, mfilename,'A',1); 

levels = varargin{2};
validateattributes(levels,{'numeric'},{'nonsparse','real','vector', ...
    'nonnan'}, mfilename,'LEVELS',2);

% Check if 'levels' in strictly increasing vector
if (length(levels) > 1) 
    diffLevels = diff(levels);
    if (any( (diffLevels <= 0) | isnan(diffLevels) )) % Two Infs in succession result in NaN. 
        error(message('images:imquantize:nonIncreasingVector','LEVELS'));    
    end
end
